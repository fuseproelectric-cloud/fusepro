...
export function CustomerDetailPageContainer() {
  ...
  const { customer, isLoading, isError, addresses, custJobs, custEstimates, custInvoices, custRequests, outstanding } =
  ...
  const createAddrMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/customers", customerId, "addresses"] }); closeAddrDialog(); },
    onError: (err: unknown) => toastErr("Could not add address", err),
  });
  const updateAddrMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: AddressForm }) => customersApi.updateAddress(customerId!, id, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/customers", customerId, "addresses"] }); closeAddrDialog(); },
  ...
  function closeAddrDialog() {
    setAddrOpen(false); setEditAddress(null);
    setAddrCity(""); setAddrState(""); setAddrZip("");
    addrForm.reset();
  }
  ...
  if (customerId == null || isError || (!isLoading && !customer)) {
    return (
    ...
    <Stack spacing={3}>
        ...
        <div className="p-6">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                {customer.company && <p className="text-sm text-muted-foreground flex items-center gap-1.5 mt-0.5"><Icon icon={Building2} size={14} /> {customer.company}</p>}
                {customer.leadSource && <p className="text-xs text-muted-foreground flex items-center gap-1.5 mt-0.5"><Icon icon={Star} size={12} /> {customer.leadSource}</p>}
                {tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {tags.map((t: string) => <span key={t} className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-100 text-blue-800">{t}</span>)}
                  </div>
                )}
              </div>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              {customer.phone && <a href={`tel:${customer.phone}`} className="flex items-center gap-1.5 h-9 px-3 rounded-md border border-border bg-card hover:bg-muted text-sm font-medium transition-colors"><Icon icon={Phone} size={14} /> Call</a>}
              {customer.email && <a href={`mailto:${customer.email}`} className="flex items-center gap-1.5 h-9 px-3 rounded-md border border-border bg-card hover:bg-muted text-sm font-medium transition-colors"><Icon icon={Mail} size={14} /> Email</a>}
              <Button onClick={openEdit} variant="outline" className="h-9"><Icon icon={Pencil} size={14} className="mr-1.5" /> Edit</Button>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-9 w-9"><Icon icon={MoreVertical} size={16} /></Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end">
                  <DropdownMenuItem className="text-destructive focus:text-destructive" onClick={() => { if (confirm(`Delete ${customer.name}?`)) deleteMutation.mutate(); }}>
          ...
          {(customer.phone || customer.email) && (
            <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-border">
              {customer.phone && <div className="flex items-center gap-2 text-sm text-muted-foreground"><Icon icon={Phone} size={14} /> {customer.phone}</div>}
              {customer.email && <div className="flex items-center gap-2 text-sm text-muted-foreground"><Icon icon={Mail} size={14} /> {customer.email}</div>}
            </div>
          )}
        </div>
        {/* Stats bar */}
        <div className="border-t border-border bg-muted/30 px-6 py-3 flex flex-wrap gap-6 rounded-b-xl">
          <div className="flex items-center gap-2"><Icon icon={ClipboardList} size={16} className="text-muted-foreground" /><span className="text-sm font-semibold">{custRequests.length}</span><span className="text-xs text-muted-foreground">Requests</span></div>
          <div className="flex items-center gap-2"><Icon icon={Briefcase} size={16} className="text-muted-foreground" /><span className="text-sm font-semibold">{custJobs.length}</span><span className="text-xs text-muted-foreground">Jobs</span></div>
          <div className="flex items-center gap-2"><Icon icon={FileText} size={16} className="text-muted-foreground" /><span className="text-sm font-semibold">{custEstimates.length}</span><span className="text-xs text-muted-foreground">Estimates</span></div>
          <div className="flex items-center gap-2"><Icon icon={Receipt} size={16} className="text-muted-foreground" /><span className="text-sm font-semibold">{custInvoices.length}</span><span className="text-xs text-muted-foreground">Invoices</span></div>
          {outstanding > 0 && <div className="flex items-center gap-2 ml-auto"><span className="text-sm font-bold text-blue-700">${outstanding.toFixed(2)}</span><span className="text-xs text-muted-foreground">Outstanding</span></div>}
          <div className="flex items-center gap-2 text-xs text-muted-foreground"><Icon icon={CalendarDays} size={14} />Client since {formatDate(customer.createdAt)}</div>
        </div>
      </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="inline-flex items-center justify-center min-w-[20px] h-5 px-1 rounded-full text-[10px] font-bold bg-muted text-muted-foreground">{addresses.length}</span>
          </div>
          <button onClick={openAddrCreate} className="flex items-center gap-1 text-xs font-medium text-blue-500 hover:text-blue-700 transition-colors">
        ...
        {addresses.length === 0 ? (
          <div className="bg-card rounded-lg border border-border border-dashed p-10 text-center">
            ...
            <button onClick={openAddrCreate} className="text-xs text-blue-500 hover:text-blue-700 font-medium">+ Add first address</button>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {addresses.map((a: CustomerAddress) => (
              <div
                ...
                className={cn("bg-card rounded-xl border cursor-pointer group transition-all hover:shadow-md hover:-translate-y-0.5 relative", a.isPrimary ? "border-blue-200 ring-1 ring-blue-200" : "border-border hover:border-blue-200")}
              ...
              >
                <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10" onClick={e => e.stopPropagation()}>
                  <button onClick={e => openAddrEdit(a, e)} className="p-1.5 rounded-md bg-background/80 border border-border hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"><Icon icon={Pencil} size={12} /></button>
                  <button onClick={e => { e.stopPropagation(); if (confirm("Delete this address?")) deleteAddrMutation.mutate(a.id); }} className="p-1.5 rounded-md bg-background/80 border border-border hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors"><Icon icon={Trash2} size={12} /></button>
                </div>
                <div className="p-5">
                  <div className="flex items-start gap-3 mb-3">
                    <div className={cn("w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0", a.isPrimary ? "bg-blue-500" : "bg-muted")}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {a.isPrimary && <span className="inline-flex mt-0.5 px-1.5 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-800">Primary</span>}
                    </div>
                  </div>
                  <div className="space-y-0.5 min-h-[40px]">
                    {(a.address || a.city) ? (
                      <a href={`https://maps.google.com/?q=${encodeURIComponent([a.address, a.city, a.state, a.zip].filter(Boolean).join(", "))}`} target="_blank" rel="noopener noreferrer" className="block hover:text-blue-500 transition-colors" onClick={e => e.stopPropagation()}>
                        {a.address && <p className="text-sm text-foreground">{a.address}</p>}
                        {(a.city || a.state || a.zip) && <p className="text-sm text-muted-foreground">{[a.city, a.state, a.zip].filter(Boolean).join(", ")}</p>}
                      </a>
                    ) : (
                      <p className="text-sm text-muted-foreground/50 italic">No address details</p>
                    )}
                  </div>
                  {a.notes && <p className="mt-2 text-xs text-muted-foreground/70 italic border-t border-border pt-2">{a.notes}</p>}
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="inline-flex items-center justify-center min-w-[20px] h-5 px-1 rounded-full text-[10px] font-bold bg-muted text-muted-foreground">{custRequests.length}</span>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {custRequests.map((req: any) => (
                  <tr key={req.id} onClick={() => navigate(`/requests?id=${req.id}`)} className="border-b border-border/60 last:border-0 hover:bg-muted/30 cursor-pointer transition-colors">
                    <td className="px-4 py-3"><p className="font-medium text-foreground">{req.title}</p><p className="text-xs text-muted-foreground">#{req.id}</p></td>
                    <td className="px-4 py-3 hidden sm:table-cell"><StatusBadge status={req.status} map={REQ_STATUS} /></td>
                    <td className="px-4 py-3 hidden md:table-cell"><StatusBadge status={req.priority} map={REQ_PRIORITY} /></td>
                    <td className="px-4 py-3 text-xs text-muted-foreground hidden lg:table-cell capitalize">{req.source ?? "—"}</td>
                    <td className="px-4 py-3 text-xs text-muted-foreground hidden lg:table-cell whitespace-nowrap">{formatDate(req.createdAt)}</td>
                    <td className="px-2 py-3"><Icon icon={ChevronRight} size={16} className="text-muted-foreground/40" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Dialog open={addrOpen} onOpenChange={o => { if (!o) { closeAddrDialog(); setAddrMapPin(null); } }} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <form onSubmit={addrForm.handleSubmit(onAddrSubmit)} noValidate className="space-y-3">
            {addrMapPin && <MiniMapPreview lat={addrMapPin.lat} lng={addrMapPin.lng} />}
            <div className="space-y-3">
              ...
              <CheckboxInput label="Set as primary address" checked={addrForm.watch("isPrimary") ?? false} onCheckedChange={v => addrForm.setValue("isPrimary", v)} />
            </div>
            <FormActions
